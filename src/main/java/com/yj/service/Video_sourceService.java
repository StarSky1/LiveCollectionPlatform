...
import com.yj.dao.HuyaLive_Video_sourceDao;
...
public class Video_sourceService {
	...
	@Autowired
	public HuyaLive_Video_sourceDao huyaLive_Video_sourceDao;
	@Autowired
	...
	public void getHuya_video_source(String live_lists_url){
		int total_page=huyaLiveSpider.getTv_videos_totalPage(live_lists_url);
		JSONObject json=huyaLiveSpider.getTv_Video_sourceBymulti_thread(live_lists_url, total_page);
		if(json==null || json.size()==0){
			System.out.println("无数据录入");
			return;
		}
		...
		//使用replace into语句批量更新直播间表所有数据，重新导入直播间数据，已获取最新直播信息
		//批量添加直播间信息
		huyaLive_Video_sourceDao.replace_source_list(source_list);
		//批量添加主播信息
		video_hostService.replace_HuyaLive_host_list(host_list);
	}
